...
func intCodeComputer(intCode []int, offset int, input []int) (int, int, bool) {
	output := input[0]
	for {
		opcode, modes := parseOptCode(intCode[offset])
		if opcode == 99 {
			return output, offset, true
		} else if opcode == 1 {
			...
			intCode[intCode[offset+1]] = input[0]
			input = input[1:]
			offset += 2
		} else if opcode == 4 {
			if modes[0] == 0 {
...
func main() {
	...
	for scanner.Scan() {
		...
		for permutations.Next() {
			...
			var offsets [5]int
			for i := 0; i < 5; i++ {
				intCodes[i] = append(intCodes[i], intCode...)
				offsets[i] = 0
			}

			for i := 0; i < 5; i++ {
				if i == 0 {
					result, offsets[i], _ = intCodeComputer(intCodes[i], offsets[i], []int{numbers[i], 0})
				} else {
					result, offsets[i], _ = intCodeComputer(intCodes[i], offsets[i], []int{numbers[i], result})
				}
			}
			...
			for {
				...
				result, offsets[i], done = intCodeComputer(intCodes[i], offsets[i], []int{result})
				if done {
					break
				}
				i++
